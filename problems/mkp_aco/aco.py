...
class ACO:
    ...
    def add_dummy_node(self):
        ...
        self.weight = torch.cat(
            (self.weight, torch.zeros((1, self.m), device=self.device)), dim=0
        )  # (n+1, m)
        self.heuristic = torch.cat(
            (self.heuristic, torch.tensor([1e-8], device=self.device))
        )  # (n+1)
    ...
    def gen_sol(self):
        ...
        knapsack = torch.zeros(
            size=(self.n_ants, self.m), device=self.device
        )  # used capacity
    ...
    def update_knapsack(self, mask, knapsack, new_item):
        ...
        for ant_idx in range(self.n_ants):
            ...
            if len(candidates) > 1:
                candidates.squeeze_()
                test_knapsack = (
                    knapsack[ant_idx].unsqueeze(0).repeat(len(candidates), 1)
                )  # (x, m)
