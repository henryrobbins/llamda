# Adapted from ReEvo: https://github.com/ai4co/reevo/blob/main/problems/cvrp/gen_inst.py
...
def generate_datasets() -> None:
    ...
    for problem_size in [50]:
        ...
        np.save(
            os.path.join(basepath, f"dataset/train{problem_size}_dataset.npy"), dataset
        )
    ...
    for problem_size in [20, 50, 100]:
        ...
        dataset = np.array(dataset)
        np.save(
            os.path.join(basepath, f"dataset/val{problem_size}_dataset.npy"), dataset
        )
    ...
    for problem_size in [20, 50, 100]:
        ...
        dataset = np.array(dataset)
        np.save(
            os.path.join(basepath, f"dataset/test{problem_size}_dataset.npy"), dataset
        )
