...
def calculate_path_cost_fitness(
    vacancies: IntArray, capacity: int
) -> Tuple[int, float]:
...
class ACO(object):
    def __init__(
        ...
        self.pheromone: FloatArray = np.ones(
            (self.problem_size, self.problem_size)
        )  # problem_size x self.problem_size
        ...
        self._ordinal: IntArray = np.arange(
            self.problem_size, dtype=int
        )  # for indexing
    ...
    def sample_path(self, prob: FloatArray) -> Tuple[
        ...
        path = (
            np.ones(self.problem_size, dtype=int) * -1
        )  # x=path[i] => put item i in bin x
